...
//Parsea JSX para trabajar con React
gulp.task("babel-parse", function () {
  return gulp.src(basePath +'js/main.js')
    .pipe(babel({
...
gulp.task('default', ['styles', 'babel-parse'], function () {
  browserSync({
    notify: false,
    server: ['.tmp', basePath]
  });
  ...
  gulp.watch([basePath +'js/main.js'], ['babel-parse', reload]);
  gulp.watch([basePath +'images/**/*'], reload);
});

//Tarea que genera el entregable en la carpeta dist
gulp.task('deploy', ['set-env-variable-prod', 'images', 'scripts', 'html'], function(){
  var filesToMove = [
      basePath + 'css/**/*.css',
